...
const extractStyleSheets = (assetsMap: StringIndexable<string> | null) => {
...
const readRemoteStyleSheet = async (styleSheet: string) => {
  let styleSheetContent = null;
  try {
    ...
    const index = styleSheetUrl.lastIndexOf('/');
    const basePathUrl = styleSheetUrl.substring(0, index + 1);
...
export const extractStyleSheetData = async (assetsMap: StringIndexable<string> | null) => {
  const styleSheets = extractStyleSheets(assetsMap);
  if (!styleSheets) return null;

  const prList = styleSheets.map((styleSheet) => {
    if (!styleSheet.startsWith('http')) {
      return readLocalStyleSheet(styleSheet);
    } else {
      return readRemoteStyleSheet(styleSheet);
    }
  });
